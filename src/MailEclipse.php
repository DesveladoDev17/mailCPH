...
class MailEclipse
{
        ...
        if (! view()->exists($view) && ! self::getTemplates()->contains('template_slug', '=', $templateName)) {
            self::saveTemplates(self::getTemplates()
                ->push([
                    ...
                    'template_agencia' => Auth::user()->cusdncvedis
                ]));
            ...
            return response()->json([
                'status' => 'ok',
                'message' => 'Template creado<br> <small>Recargando...<small>',
                'template_url' => route('viewTemplate', ['templatename' => $templateName]),
            ]);
        }
    ...
    public static function generateMailable($request = null): JsonResponse
    {
        ...
        if (! $request->has('force') && ! self::getMailable('name', $name)->isEmpty()) {
            return response()->json([
                'status' => 'error',
                'message' => 'Este nombre de correo ya existe. ¡Los nombres deben ser únicos! para anularlo, habilite la opción "forzar".',
            ]);
        }
        ...
        if (strtolower($name) === 'mailable') {
            return response()->json([
                'status' => 'error',
                'message' => 'No puede utilizar "mailable" como nombre de correo',
            ]);
        }

        $name = $customPath ? $customPath.'/'.$name : $name;

        $name = 'agencia'.Auth::user()->cusdncvedis.'/'. $name;
        $params = collect([
            'name' => $name,
        ]);
        ...
        if ($exitCode > -1) {
            return response()->json([
                'status' => 'ok',
                'message' => 'Enviable creado<br> <small>Recargando...<small>',
            ]);
        }

        return response()->json([
            'status' => 'error',
            'message' => 'Enviable creado con éxito',
    ...
    {
        $fqcns = [];
        $url_mailing = config('maileclipse.mailables_dir') . 'agencia'. Auth::user()->cusdncvedis;

        if (! file_exists($url_mailing)) {
            return;
        } else {
            $url_mailing = config('maileclipse.mailables_dir') . 'agencia'. Auth::user()->cusdncvedis;
       
            $allFiles = new RecursiveIteratorIterator(new RecursiveDirectoryIterator($url_mailing));
