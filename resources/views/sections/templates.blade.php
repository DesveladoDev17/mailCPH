...
<div class="col-lg-10 col-md-12">
                ...
                <div class="card my-4">
                    <div class="card-header d-flex align-items-center justify-content-between"><h5>{{ __('Templates') }}</h5>
                        ...
                        <a href="{{ route('selectNewTemplate') }}" class="btn btn-primary">{{ __('Nuevo Template') }}</a>
                        @endif
                    </div>
                    ...
                    @component('maileclipse::layout.emptydata')
                        ...
                        <span class="mt-4">{{ __("No encontramos nada, sólo espacio vacío..") }}</span>
                        <a class="btn btn-primary mt-3" href="{{ route('selectNewTemplate') }}">{{ __('Nuevo Template') }}</a>
                    ...
                    <table id="templates_list" class="table table-responsive table-hover table-sm mb-0 penultimate-column-right">
                        ...
                        @foreach($templates->all() as $template)
                           @if($template->template_agencia === Auth::user()->cusdncvedis)
...
<script type="text/javascript">
    ...
    notie.confirm({
        ...
        text: 'Estás segura de que quieres hacer eso?<br>Eliminar Template <b>'+ templateName +'</b>',
        submitText: 'Si',
        cancelText: 'Cancelar',
    ...
    .then(function (response) {
        ...
        if (response.data.status == 'ok'){
            notie.alert({ type: 1, text: 'Template eliminado', time: 2 });
            ...
            notie.alert({ type: 'error', text: 'Template no eliminado', time: 2 })
        }
    })
