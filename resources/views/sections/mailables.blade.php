...
<div class="col-lg-10 col-md-12">
                ...
                <div class="card my-4">
                    <div class="card-header d-flex align-items-center justify-content-between"><h5>{{ __('Enviables') }}</h5>

                        @if (!$mailables->isEmpty())
                            <a class="btn btn-primary" href="#newMailableModal" data-toggle="modal" data-target="#newMailableModal">{{ __('Nuevo enviable') }} </a>
                        @endif
                        <!-- Modal -->
                    </div>
                    ...
                    @component('maileclipse::layout.emptydata')
                        ...
                        <span class="mt-4">{{ __("No encontramos nada, sólo espacio vacío.") }}</span><button class="btn btn-primary mt-3" data-toggle="modal" data-target="#newMailableModal">{{ __('Añadir nuevo enviable') }}</button>
                    ...
                    <table id="mailables_list" class="table table-responsive table-hover table-sm mb-0 penultimate-column-right">
                        <thead>
                            <tr>
                                ...
                                <th scope="col">{{ config('maileclipse.display_as_subject') ? __('Asunto') : __('Namespace')}}</th>
                                <th scope="col">{{ __('Última edición') }}</th>
                                <th scope="col"></th>
                            </tr>
                        </thead>
                        <tbody>
  ...
  <form id="create_mailable" action="{{ route('generateMailable') }}" method="POST">
    ...
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Nuevo correo enviable</h5>
      ...
      <div class="modal-body">
        <div class="alert alert-warning new-mailable-alerts d-none" role="alert">
          ...
          <div class="form-group">
            <label for="mailableName">Nombre</label>
            <input type="text" class="form-control" id="mailableName" name="name" placeholder="Mailable name" required>
            <small class="form-text text-muted">Poner nombre al correo para enviare.j <b>Welcome User</b>, <b>WelcomeUser</b></small>
          </div>
          <div class="form-group">
            <label class="checkbox-inline">
                <input type="checkbox" id="markdown--truth" value="option1">Markdown del Template
                <small class="form-text text-muted">Usar markdown template</small>
            </label>
        </div>
        ...
        <div class="form-group">
            <label class="checkbox-inline">
                <input type="checkbox" id="forceCreation" name="force"> Forzar
                <small class="form-text text-muted">Forzar la creación de correo incluso si ya existe</small>
            </label>
        </div>
        </form>
      </div>
      <div class="modal-footer">
        ...
        <button type="submit" class="btn btn-primary">Crear correo enviable</button>
      </div>
    </div>
</form>
  </div>
...
<script type="text/javascript">
    ...
    notie.confirm({
        ...
        text: 'Estás seguro de que quieres hacer eso?<br>Eliminar enviable <b>'+ mailableName +'</b>',
        submitText: 'Si',
        cancelText: 'Cancelar',
